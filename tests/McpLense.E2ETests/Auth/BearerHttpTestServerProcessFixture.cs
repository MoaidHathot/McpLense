...
/// <c>--require-bearer &lt;token&gt;</c>, so every request must carry the matching
/// bearer header. Used by bearer auth subprocess CLI tests.
/// </summary>
public sealed class BearerHttpTestServerProcessFixture : IAsyncLifetime
{
    public const string TestToken = "e2e-bearer-token-xyz789";
    ...
    {
        _urlFilePath = Path.Combine(Path.GetTempPath(), $"mcplense-bearer-httptest-{Guid.NewGuid():N}.url");
        ...
        psi.ArgumentList.Add("--require-bearer");
        psi.ArgumentList.Add(TestToken);
        ...
        if (!_process.Start())
        {
            throw new InvalidOperationException("Failed to start McpLense.TestHttpServer (bearer) subprocess.");
        }
    ...
    private async Task WaitForUrlFileAsync(string path, TimeSpan timeout)
    {
        ...
        while (DateTime.UtcNow < deadline)
        {
            if (_process is { HasExited: true })
            {
                throw new InvalidOperationException(
                    $"McpLense.TestHttpServer (bearer) subprocess exited before publishing a URL. " +
        ...
        throw new TimeoutException(
            $"McpLense.TestHttpServer (bearer) did not publish a URL to '{path}' within {timeout}. " +
